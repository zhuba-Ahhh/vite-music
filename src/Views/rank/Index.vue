    ...
    <div class="rank">
        <div class="rank-container">
            <div class="rank-main">
                    ...
                    <div class="cover-info">
                        <div class="cover-header">
                            <div class="cover-title">
                                {{ rankInfo.name }} <span>({{ formartDate(rankInfo.updateTime, 'MM月dd日') }}
                                    更新)</span>
                            </div>
                        </div>
                        <div class="cover-author-tags">
                            <div class="cover-author" v-if="rankInfo.creator">
                                ...
                                <div class="cover-date">{{ formartDate(rankInfo.createTime, 'yyyy-MM-dd') }}</div>
                            </div>
                        </div>
                ...
                <div class="song-main">
                    <div class="song-header">
                        ...
                        <span :class="['collect', rankInfo.subscribed ? 'active' : '']" @click="subPlayList(rankInfo)"><i
            ...
            <div class="rank-aside">
                ...
                <div class="type-main">
                    <div class="type-item" :class="rId == item.id ? 'active' : ''" v-for="(item, index) in list"
...
import { formatSongs, formartDate, formartNum } from '../../utils';
import { ElMessage } from 'element-plus';
import { topListDetail, listDetail, List1 } from '../../apis';
...
let list = reactive<List1[]>([]);
let listFeature = reactive<List1[]>([]);
let listOther = reactive<List1[]>([]);
let listTop = reactive<List1[]>([]);
let songList = reactive<any[]>([]);
let rankInfo = reactive<any>({});

const getTopListDetail = async () => {
    const { data } = await topListDetail();
    const { code, list: List } = data;
    ...
    // 云音乐TOP榜
    listTop = List.filter(item => {
        return item.toplistType
    })
    // 云音乐特色榜
    listFeature = List.filter(item => {
        return !item.toplistType && item.name.indexOf('云音乐') >= 0
    })
    // 其他场景榜
    listOther = List.filter(item => {
        return !item.toplistType && item.name.indexOf('云音乐') < 0
    })
    list = type.value ? listTop : listTop;
    rId.value = rId.value ? rId.value : listTop[0].id
};

const getListDetail = async () => {
    isLoading.value = true;
    const { data } = await listDetail({ id: String(info.rId), s: -1 });
    const { code, playlist, privileges } = data;
    ...
    rankInfo = playlist
    songList = formatSongs(playlist.tracks, privileges)
    total.value = info.songList.length
    isLoading.value = false
};

const selectType = (type: { id: string | number; }[]) => {
    type.value = type
    list = list + type.value
    rId.value = list + type[0].id
...
onMounted(() => {
    type.value = String(route.query.type) || info.type;
    rId.value = Number(route.query.rId);
    getTopListDetail();
});
