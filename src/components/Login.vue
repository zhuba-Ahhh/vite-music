...
import { login, getUserInfo as userInfo } from '../apis';
...
const loginFormRef = ref<FormProps | null>(null);
...
const submitForm = () => {
    loginFormRef?.value?.validate(async (valid) => {
        if (valid) {
            const { data: res } = await login(formInfo['loginForm']);
...
const getUserInfo = async (uid: string) => {
    const { data: res } = userInfo({ uid: uid })
